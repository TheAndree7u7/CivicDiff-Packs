...
import { validateDigest, validateSelfcheck } from "@/lib/shared/schemas"
import type { Digest } from "@/lib/shared/schemas"
import { checkRateLimit, getClientId, RATE_LIMITS } from "@/lib/server/rate-limit"
...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      const { getGeminiConfig, generateSelfcheck } = await import("@/lib/server/gemini")
      const config = getGeminiConfig()
      if (config.hasKey && packId) {
        const { loadPackData } = await import("@/lib/server/pipeline")
